...
{
    public partial class UcSqlmonPolicy : UserControl
    {
        private static readonly Lazy<UcSqlmonPolicy> lazy =
            new Lazy<UcSqlmonPolicy>(() => new UcSqlmonPolicy(), LazyThreadSafetyMode.ExecutionAndPublication);

        public static UcSqlmonPolicy Instance { get { return lazy.Value; } }

        LogClient.Config logClientConfig = LogClient.Config.Instance;
        DataManager dataManager = DataManager.Instance;
        FileDb fileDb = FileDb.Instance;

        public UcSqlmonPolicy()
        {
            InitializeComponent();
            InitDgv();
        ...
        private async void LoadData(object sender, EventArgs e)
        {
            try
            {
                ...
                textBox00Comment.Text = "Collects completed query statistics and writes them to the lazylog database. The recorded value is the delta value." + Environment.NewLine + "Recommended value 60 seconds";
                ...
                textBox01Comment.Text = "Collects the progress of the query currently being performed. You can see which query was running in SQL Server." + Environment.NewLine + "Recommended value 30 seconds";
                ...
                textBox02Comment.Text = "Collects lock information for the database." + Environment.NewLine + "Recommended value 3 seconds";
                ...
                textBox03Comment.Text = "Can be stored for at least 3 minutes to 3 days(4320 minutes)" + Environment.NewLine + "Recommended value 1440 minutes";
        ...
        private async void buttonLoadPolicy_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Task<string> dm_exec_query_stats_ProbeIntervalSec = dataManager.Execute
                    ("ExecuterRest"
                    , "TypeConfigRead"
                    , cmdText
                ...
                Tasks.Add(dm_exec_query_stats_ProbeIntervalSec);
                ...
                cmdText = JsonConvert.SerializeObject(typeConfigRead);
                Task<string> dm_os_workers_ProbeIntervalSec = dataManager.Execute
                    ("ExecuterRest"
                    , "TypeConfigRead"
                    , cmdText
                    , CsLib.RequestType.POST
                    ...
                    , LogClient.Config.Instance.GetValue(LogClient.Category.Api, LogClient.Key.AccessKey)
                    , LogClient.Config.Instance.GetValue(LogClient.Category.Api, LogClient.Key.SecretKey));
                Tasks.Add(dm_os_workers_ProbeIntervalSec);
                ...
                wcfResponse = JsonConvert.DeserializeObject<WcfResponse>(dm_exec_query_stats_ProbeIntervalSec.Result);
                if (wcfResponse.IsSuccess)
                {
                    ...
                    textBox_dm_exec_query_stats_IntervalSec.Text = typeConfigRead.Value;
                }
                else
                    throw new Exception(wcfResponse.ErrorMessage);

                wcfResponse = JsonConvert.DeserializeObject<WcfResponse>(dm_os_workers_ProbeIntervalSec.Result);
                if (wcfResponse.IsSuccess)
                {
                    string resultMessage = wcfResponse.ResultMessage;
                    typeConfigRead = JsonConvert.DeserializeObject<TypeConfigRead>(resultMessage);
                    textBox_dm_os_workers_IntervalSec.Text = typeConfigRead.Value;
                }
                else
                    throw new Exception(wcfResponse.ErrorMessage);
                ...
                if (wcfResponse.IsSuccess)
                {
                    string resultMessage = wcfResponse.ResultMessage;
                    typeConfigRead = JsonConvert.DeserializeObject<TypeConfigRead>(resultMessage);
                    textBox_sp_lock2_intervalSec.Text = typeConfigRead.Value;
                }
                else
                    throw new Exception(wcfResponse.ErrorMessage);
        ...
        private async Task ApplyPolicy(string ip)
        {
            try
            {
                ...
                if (!wcfResponse.IsSuccess)
                    throw new Exception("Sqlmon Manager Stop Failed");
                ...
                if (!wcfResponse.IsSuccess)
                    throw new Exception("Sqlmon Manager Start Failed");
