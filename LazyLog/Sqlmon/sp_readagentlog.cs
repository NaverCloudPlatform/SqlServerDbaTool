...
namespace lazylog
{
    class sp_readagentlog : BaseSqlmon<sp_readagentlog_data>
    {
        ...
        internal override void Start()
        {
            ...
            while (IsRunning)
            {
                try
                {
                    ...
                    {
                        IsRunning = false;
                        log.Warn("sqlmon (sp_readagentlog) can't start because ProbeIntervalSec is 0");
                        break; 
                    }
                    else
                    {
            ...
            try
            {
                ...
                if (!GetCurrentTableName(BaseTableName, out CurrentTableName))
                {
                    Common.QueryExecuter(
                        ...
                        , string.Format(TableGenQuery, BaseTableName, "ErrorLevel"));
...
", BaseTableName);
                    ...
                    if (xpLogNewBuffer.Count > 0)
                    {
                        if (!SenderType.Equals("No", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (SenderType.Equals("A", StringComparison.OrdinalIgnoreCase))
                                sender = new sp_readagentlogTypeA(xpLogNewBuffer);
                            else
                                sender = new sp_readagentlogTypeB(xpLogNewBuffer);
                        ...
                        {
                            buffer.Add(new sp_readagentlog_data
                            {
                                LogDate = a.Key.Item1,
        ...
        protected override void Initialization()
        {
            try
            {
                RunIntervalSec = Convert.ToInt32(config.GetValue(Category.Sqlmon, Key.sp_readagentlog_ProbeIntervalSec));

                GetDataQuery = @"
set nocount on 
set transaction isolation level read uncommitted 
exec xp_readerrorlog 0, 2, null, null, @LogStartTime, null, 'asc'
";
